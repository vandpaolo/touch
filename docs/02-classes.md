...
| `touch_backend.session` | Per-WS-connection state: the canonical **op-history** `TouchDocument`, conv state, cancel token, mesh cache. Rebuild routes through the derived Layer Stack (TP1 bridge) | `Session` class | server | document, planner, layer_bridge, live_build, frames, mesh_cache |
...
| `touch_backend.operation_adapter` | Operation history → build123d source (pure, deterministic; F24/N10). Per-op emitters thread a solid var; the chamfer is **finder-scoped** via `resolve_face` (not the whole face loop). `rhs()` exposes the per-op RHS so the Layer Stack bridge builds layer source from the same emitters. Op set: box/cylinder/sphere/chamfer. *(The Intent-based `adapters.build123d_target` is the retained Maquette-era sibling.)* | `emit(history) -> str`, `rhs(operation, prev_var) -> str` | session, layer_bridge | _generated.protocol, adapters, re |
| `touch_backend.agent.executor` | Run emitted build123d source in a **workspace-confined** subprocess (F46, ADR-0016): network-off + write-guard preamble, secret-scrubbed env, timeout + SIGKILL grace, soft import-lint → `part.step`. The single execution chokepoint; **stdlib-only**, imports no other `touch_backend` module | `Executor.execute(code_path) -> ExecutionResult` (incl. `warnings`) | live_build | subprocess, stdlib |
...
| `touch_backend.layer_stack` (pivot) | The active part as an ordered list of `Layer`s: deterministic fold (`rebuild` via an **injected `build` callable** — stays OCP-free) + per-layer content cache; **versioned** + compare-and-swap; layer-native (de)serialization; append-only v0 (ADR-0012/0013). *CAS + layer-native persistence are built **capabilities**, not yet wired live — see the TP1 bridge note in 02-architecture.md.* | `LayerStack`, `Layer.from_template/from_code`, `add_layer(layer, *, expect_rev)`, `delete_last(*, expect_rev)`, `revision`, `rebuild(*, build, cache)`, `emit` / `emit_layerwise`, `to_dict` / `from_dict`, `RebuildResult`, `StaleRevisionError` | layer_bridge, live_build, templates | mesh_cache, _generated.protocol |
| `touch_backend.provenance` (pivot) | Per-face attribution by **trim-independent** geometric diff (`BRepAdaptor` surface keys) → `created_by`/`last_modified_by` **sets**, baked into the Mesh face ids (F39, ADR-0012). Faces only in v0; single-owner on a fused face is a known R-B limit | `attribute(prev, next, *, layer_id, prior) -> ProvenanceMap`, `attribute_stack(solids, layer_ids)`, `bake(mesh, prov)`, `ProvenanceMap` / `ProvenanceEntry` | live_build | finder, OCP |
| `touch_backend.templates` (pivot) | Recognise Touch's **own emitted** template shapes (box/cylinder/sphere/chamfer) → parametric card, else code card. Exact byte-shape match only, never decompilation (F40) | `recognize(source: str) -> Recognized \| None`, `Recognized` | layer_bridge | re, layer_stack (`Template`) |
| `touch_backend.layer_bridge` (pivot) | **TP1 bridge:** derive a `LayerStack` from the op-history (each op → a layer via `operation_adapter.rhs` threading `body`; box/cylinder/sphere → template, finder-scoped chamfer → code) — a **pure function of the history, re-derived per rebuild** (not stored). Also layer-native `.touch` save/load + op-history→stack migration | `layers_from_history(history)`, `layer_from_operation(op)`, `save_stack(stack, path)`, `load_stack(path)` | session | operation_adapter, templates, layer_stack, document |
| `touch_backend.live_build` (pivot) | Build the live mesh for a `LayerStack` with provenance baked in: `emit_layerwise` → `Executor` → import per-layer solids → `provenance.attribute_stack` → `tessellate` → `bake` (F39). Provenance is best-effort (R-B); OCP imported lazily | `build_mesh(stack, *, timeout_s) -> Mesh`, `GeometryError` | session | layer_stack, agent.executor, tessellate, provenance, build123d |
| `touch_backend.mcp_server` (pivot) | **NOT BUILT — TP2.** Claude-Code-spawned stdio MCP server; forwards to the running backend; geometry tools; structured mutating-tool envelope (ADR-0014) | *(planned)* `tools: get_model_state/get_selection/render_view/list_layers/get_layer/add_layer/edit_layer/reorder_layer/delete_layer` | (Claude Code) | server (WS client), context_packets |
| `touch_backend.context_packets` (pivot) | **NOT BUILT — TP2.** Build the **positional** vs **macro** context packets injected for the agent (F45, ADR-0015) | *(planned)* `positional(selection) -> dict`, `macro(stack) -> dict` | mcp_server, session | layer_stack, finder |
