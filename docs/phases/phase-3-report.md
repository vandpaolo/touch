...
|---|---|---|
| 1 — loop API-error hardening (P3-Q1) + `cli.py` + contract + entry point | done | [`loop._plan`](../../src/maquette/agent/loop.py) catches `anthropic.AnthropicError` → `PLANNING_FAILED` / exit 10 + complete run folder; [`src/maquette/cli.py`](../../src/maquette/cli.py) (`design` command; F14 flags → `Config.load` → `RunConfig`; `Anthropic()` + `PromptsBundle`; `Loop.run`; reads `status.json.exit_code`; prints run dir F12; missing-key → exit 1); `[project.scripts] maquette`; `[tool.importlinter]` `cli` contract (`allow_indirect_imports`). Commit `5fcc588`. |
