...
| R10 | **OCP / OCCT license redistribution.** OCP is an LGPL-2.1+ Python binding over OpenCASCADE (LGPL). The Windows installer that bundles OCP's native libs is redistributing LGPL code; LGPL imposes notice + relinking obligations on the redistributor. | Day 5 include a `LICENSES/` directory in the installer with the OCP + OCCT license texts; the installer welcome screen + README reference them; verify the open-source redistribution path needs no commercial sublicense. If unclear at any point: file `/pm-blocker` and consult before any release-tag push (no public release until this is settled). |
| R11 | **AV false-positives on unsigned PyInstaller bundles.** Windows Defender and several third-party AV products have a documented history of flagging unsigned PyInstaller `.exe`s as PUA / trojan — distinct from SmartScreen (R7). Could quarantine the sidecar mid-install or at first launch. | Day 6 test against the VM's default Defender. If quarantined: capture the AV report, submit a Microsoft false-positive report, document the friend-side workaround (Defender exclusion); signing in T13 is the structural fix. If unfixable for the spike, phase report logs it as a v0 caveat ("friends may need to allow the install in their AV"). |
| R12 | **OCP wheel Python-version pin vs PyInstaller-bundled Python.** OCP wheels are built against a specific Python minor (e.g. 3.12.x); the PyInstaller-bundled Python on the Windows runner must match *exactly* or the wheel fails to import after packaging. Distinct from R1's binary-collection issue. | Pin the Python version in `pyproject.toml` (`requires-python = "==3.12.*"`); pin the GH Actions Windows runner's `setup-python` action to the matching minor; document the pin in `spike/sidecar/README.md`. Day 4 standalone-binary test would catch a version mismatch on Linux first. |
| R13 | **NSIS installer scope (perMachine vs perUser).** Default NSIS often installs perMachine, which prompts UAC — non-admin friends can't install. | Explicitly set `nsis.perMachine: false` + `nsis.oneClick: false` in `electron-builder` config so the install lands in `%LOCALAPPDATA%` without UAC. Day 5 verifies the installer is non-admin on the GH runner; Day 6 confirms on the VM logged in as a standard user. |
| R14 | **Even signed installers without reputation trip SmartScreen.** Code-signing alone doesn't bypass SmartScreen Filter Reputation — only EV certificates instantly establish reputation; standard OV certs build reputation over hundreds of downloads. R7 deferred "signing" to T13 without noting this nuance. | T0 deliberately defers full mitigation to T13. T0's phase report explicitly captures this caveat for T13's planning: T13 must pick EV vs OV and price the reputation-build cost. No T0-side action beyond documenting. |
