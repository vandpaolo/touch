...
| 2 — `render/orthographic.py` | done | [`render.orthographic`](../../src/maquette/render/orthographic.py) → 3 off-screen PNGs (front/side/top) via vtk-osmesa (N5); committed fixture STEP; [`test_render.py`](../../tests/test_render.py) (incl. geometry-not-blank guard); pyproject + CI swap to `vtk-osmesa`. Commits `94c10ce`, `da63d0e`. |
| 3 — `agent/executor.py` | done | [`Executor.execute`](../../src/maquette/agent/executor.py) (subprocess `cwd=run dir`, timeout + SIGTERM→SIGKILL grace N9, STEP capture, structured `error.json` N6, run with `python -I`); `ExecutionResult` (no `renders`); [`test_executor.py`](../../tests/test_executor.py). Exit codes 0/12/13. Commit `71f6d12`. |
| 4 — `agent/loop.py` | done | [`Loop.run`](../../src/maquette/agent/loop.py) + `RunConfig`: state machine, run-id (F11), planner→contract-validation→sanity→worker→executor→render wiring, `trace.jsonl` (F10), `status.json` (F9, cost via `pricing`, `prompts_hash` ADR-0003), error.json routing; [`test_loop.py`](../../tests/test_loop.py) (6 paths, real worker/executor/render). Commit `1ed938c`. |
